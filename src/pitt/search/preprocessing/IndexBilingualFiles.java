package pitt.search.preprocessing;
...
public class IndexBilingualFiles {
	...
	if (!docDir1.exists()) {
		...
		if (!docDir1.isDirectory()) {
		    System.out.println("Test id directory failed: " + docDir1);
		    System.exit(1);
		}
	    }
	}
	...
	if (!docDir2.exists()) {
		...
		if (!docDir2.isDirectory()) {
		    System.out.println("Test id directory failed: " + docDir2);
		    System.exit(1);
		}
	    }
	}
	...
	try {
		...
		try {
		    writer.addDocument(fileBilingualDocument(new File(LANGUAGE1 + "/" + files1[i]), 
							     new File(LANGUAGE1 + "/" + files2[i])));
		}
		catch (IOException e) {
